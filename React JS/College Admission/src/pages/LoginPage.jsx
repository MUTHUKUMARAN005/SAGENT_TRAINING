...
import { findLinkedStudentForUser, isStudentRoleUser } from '../utils/ownership';
...
const LoginPage = () => {
  ...
  const getStudentId = (student) =>
    student?.studentID || student?.studentId || student?.id || null;

  const buildStudentCreatePayloads = (authPayload) => {
    const name = normalizeText(authPayload?.fullName || authPayload?.name || authPayload?.username || 'Student');
    const email = normalizeText(authPayload?.email || '');
    const phone = normalizeText(authPayload?.phone || authPayload?.mobile || '0000000000');
    const dateOfBirth = normalizeText(authPayload?.dateOfBirth || authPayload?.dob || '2000-01-01');

    const payloads = [
      { name, email, phone, dateOfBirth },
      { fullName: name, email, phone, dateOfBirth },
      { studentName: name, email, phone, dateOfBirth },
      { name, email, mobile: phone, dateOfBirth },
    ];
  ...
  const ensureStudentProfileLinked = async (authPayload) => {
    if (!authPayload || !isStudentRoleUser(authPayload)) {
      return authPayload;
    }

    const attachStudentId = (payload, student) => {
    ...
    try {
      const currentStudentsRes = await API.get('/students');
      const currentStudents = Array.isArray(currentStudentsRes.data) ? currentStudentsRes.data : [];
      const linkedStudent = findLinkedStudentForUser(currentStudents, authPayload);
      if (linkedStudent) {
        return attachStudentId(authPayload, linkedStudent);
      }

      const studentPayloads = buildStudentCreatePayloads(authPayload);
      for (const payload of studentPayloads) {
        try {
          ...
          const status = error?.response?.status;
          if (![400, 403, 404, 409, 415, 422, 500].includes(status)) {
            throw error;
          }
        }
      }

      const refreshedStudentsRes = await API.get('/students');
      const refreshedStudents = Array.isArray(refreshedStudentsRes.data) ? refreshedStudentsRes.data : [];
      const refreshedLinkedStudent = findLinkedStudentForUser(refreshedStudents, authPayload);
      if (refreshedLinkedStudent) {
        return attachStudentId(authPayload, refreshedLinkedStudent);
      }

      const fallbackByEmail = refreshedStudents.find(
        (student) =>
          normalizeText(student?.email).toLowerCase()
          && normalizeText(student?.email).toLowerCase() === normalizeText(authPayload?.email).toLowerCase()
      );
      if (fallbackByEmail) {
        return attachStudentId(authPayload, fallbackByEmail);
      }
    } catch {
      // Login should not fail due linking flow.
    }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (authPayload) {
        const linkedAuthPayload = await ensureStudentProfileLinked(authPayload);
        login(linkedAuthPayload);
        toast.success(`Welcome back, ${linkedAuthPayload.fullName || linkedAuthPayload.username || 'User'}! 🎉`);
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (authPayload) {
        const linkedAuthPayload = await ensureStudentProfileLinked(authPayload);
        login(linkedAuthPayload);
        toast.success(`Welcome, ${linkedAuthPayload.fullName || linkedAuthPayload.username || 'User'}! 🚀`);
