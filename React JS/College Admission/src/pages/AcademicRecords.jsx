...
const AcademicRecords = () => {
  ...
  const canCreateRecord = hasPermission('RECORD_CREATE');
  const canDeleteRecord = hasPermission('RECORD_DELETE');
  const visibleStudents = filterStudentsForUser(students, user);
  const visibleRecords = filterRecordsForUser(records, user, (record) => record?.student);
    ...
    <PageTransition>
      <div className="page-header">
        ...
        >
          ...
          <p>{isStudentUser ? 'View your academic history' : 'View and manage student academic history'}</p>
        </motion.div>
        {canCreateRecord && (
          <motion.button
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '1.4rem', fontWeight: 800 }}>
                  {visibleRecords.filter(r => r.grade === grade).length}
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <AnimatedTable
        title={`${isStudentUser ? 'My Academic Records' : 'Academic Records'} (${visibleRecords.length})`}
        columns={columns}
        data={visibleRecords}
        actions={canDeleteRecord ? ((row) => (
          <motion.button
