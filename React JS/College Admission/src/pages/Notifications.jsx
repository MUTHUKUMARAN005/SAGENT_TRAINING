...
const Notifications = () => {
  ...
  const canDeleteNotification = hasPermission('NOTIFICATION_DELETE');
  const visibleStudents = filterStudentsForUser(students, user);
  const visibleNotifications = filterRecordsForUser(notifications, user, (n) => n?.student);
  ...
  return (
    <PageTransition>
      <div className="page-header">
        ...
        >
          <h1>🔔 Notifications</h1>
          <p>{isStudentUser ? 'View your notifications' : 'Manage and send notifications to students'}</p>
        </motion.div>
      ...
      <AnimatedTable
        title={`${isStudentUser ? 'My Notifications' : 'Notifications'} (${visibleNotifications.length})`}
        columns={columns}
        data={visibleNotifications}
        actions={canDeleteNotification ? ((row) => (
          <motion.button
