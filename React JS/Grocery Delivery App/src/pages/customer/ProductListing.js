...
const ProductListing = () => {
  ...
  let filtered = products.filter((p) => {
    ...
    const matchMin = min === null || Number.isNaN(min) || productPrice >= min;
    const matchMax = max === null || Number.isNaN(max) || productPrice <= max;
            ...
            <div className={`plp-products ${viewMode}`}>
              <AnimatePresence mode="popLayout">
                {filtered.map((product, i) => (
                  <ProductCard key={product.productId || product.id} product={product} index={i} />
                ))}
              </AnimatePresence>
            </div>
          )}
