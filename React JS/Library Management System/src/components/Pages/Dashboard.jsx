...
const Dashboard = () => {
  ...
  const normalizedStats = {
    ...
    issuedBooks: stats.issuedBooks ?? stats.activeBorrowings ?? 0,
    availableBooks: stats.availableBooks ?? stats.availableCopies ?? 0,
    totalStudents: stats.totalStudents ?? stats.totalMembers ?? 0,
    categoriesCount: stats.categoriesCount ?? stats.totalCategories ?? 0,
  ...
  const adminStats = [
    { key:'totalBooks', label:'Total Books', icon:'BK', color:'#0ea5a4', link:'/books' },
    { key:'issuedBooks', label:'Issued Books', icon:'IS', color:'#f59e0b', link:'/borrowings' },
    { key:'availableBooks', label:'Available Books', icon:'AV', color:'#22c55e', link:'/books' },
    { key:'totalStudents', label:'Total Students', icon:'MB', color:'#3b82f6', link:'/members' },
    { key:'unpaidFines', label:'Unpaid Fines', icon:'FN', color:'#f97316', link:'/fines' },
    { key:'pendingRequests', label:'Pending Requests', icon:'RQ', color:'#8b5cf6', link:'/requests' },
    { key:'categoriesCount', label:'Categories', icon:'CT', color:'#06b6d4', link:'/categories' },
  ];

  const memberStats = [
    { key:'totalBooks', label:'Books Available', icon:'BK', color:'#0ea5a4', link:'/books' },
    { key:'activeBorrowings', label:'My Active Borrows', icon:'BR', color:'#f59e0b', link:'/borrowings' },
    { key:'unpaidFines', label:'My Fines', icon:'FN', color:'#ef4444', link:'/fines' },
    { key:'pendingRequests', label:'My Requests', icon:'RQ', color:'#8b5cf6', link:'/requests' },
  ];
    ...
    <PageTransition>
      ...
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.3 }}
        style={{ marginBottom:'20px', padding:'12px 18px',
          background:'rgba(14, 165, 164, 0.08)', border:'1px solid rgba(45, 212, 191, 0.22)',
          borderRadius:'12px', display:'flex', alignItems:'center', gap:'10px' }}>
        <span style={{ fontSize:'11px', fontWeight:800, color:'#8be9dd' }}>RBAC</span>
        <span style={{ fontSize:'11px', color:'#94a3b8' }}>
          Role: <span style={{ color:rc?.color, fontWeight:700 }}>{rc?.label}</span> •
          You have access to <span style={{ color:'#2dd4bf', fontWeight:700 }}>{visibleStats.length}</span> dashboard metrics
        </span>
      </motion.div>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                ...
                <div style={{ fontSize:'30px', fontWeight:800, color:card.color }}>
                  <AnimatedCounter end={normalizedStats[card.key]||0} />
                </div>
              </div>
              <motion.div whileHover={{ scale:1.15, rotate:8 }}
                style={{ width:'48px', height:'48px',
                  ...
                  fontSize:'11px', fontWeight:800, color:'#f6fbff', boxShadow:`0 6px 20px -6px ${card.color}50` }}>{card.icon}</motion.div>
            </div>
            <div style={{ marginTop:'12px', height:'3px', background:'rgba(255,255,255,0.04)',
              borderRadius:'2px', overflow:'hidden' }}>
              <motion.div initial={{ width:0 }}
                animate={{ width:`${Math.min(((normalizedStats[card.key]||0)/10)*100,100)}%` }}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'20px' }}>
            <motion.div animate={{ scale:[1,1.06,1] }} transition={{ duration:2, repeat:Infinity }}
              style={{ width:'60px', height:'60px', background:'linear-gradient(135deg, #f59e0b, #ef4444)',
                ...
                fontSize:'11px', fontWeight:800, color:'#fff8eb', boxShadow:'0 6px 25px rgba(245,158,11,0.3)' }}>DUE</motion.div>
            <div>
              ...
              <div style={{ fontSize:'36px', fontWeight:900, color:'#f59e0b' }}>
                $<AnimatedCounter end={Math.floor(normalizedStats.totalUnpaidAmount||0)} duration={2200} />
                <span style={{ fontSize:'18px', color:'rgba(245,158,11,0.5)' }}>
                  .{String(Math.round(((normalizedStats.totalUnpaidAmount||0)%1)*100)).padStart(2,'0')}
                </span>
              </div>
            </div>
          </div>
      ...
      <PermissionGate permission={PERMISSIONS.CREATE_REQUEST}>
        <div style={{ marginTop:'24px' }}>
          ...
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
            {[
              { label:'Browse Books', icon:'BK', path:'/books', color:'#0ea5a4' },
              { label:'My Borrowings', icon:'BR', path:'/borrowings', color:'#f59e0b' },
              { label:'New Request', icon:'RQ', path:'/requests', color:'#8b5cf6' },
              { label:'My Profile', icon:'PF', path:'/profile', color:'#3b82f6' },
            ].map((a,i) => (
