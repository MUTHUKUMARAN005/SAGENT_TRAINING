...
const Members = () => {
  ...
  const canViewAllMembers = checkPermission(PERMISSIONS.VIEW_MEMBERS);

  const filterMembersByRole = useCallback((rows, currentUser) => {
    if (canViewAllMembers) return rows;
    if (!currentUser) return [];

    const currentEmail = currentUser.email?.toLowerCase();

    return rows.filter((member) => (
      (currentUser.memberId && member.memberId === currentUser.memberId) ||
      (currentEmail && typeof member.email === 'string' && member.email.toLowerCase() === currentEmail)
    ));
  }, [canViewAllMembers]);

  return (
  <CrudPage config={{
    entityName: canViewAllMembers ? 'Student' : 'Member', icon:'👥', gradient:'linear-gradient(135deg,#10b981,#34d399)', color:'#10b981',
    ...
    filterData: filterMembersByRole,
